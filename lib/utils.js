...
Basiclytics.Utils.sendData = function(event_type, data) {
  ...
  img.src = Basiclytics._APIendpoint+'?ts='+Basiclytics.Utils.now()+'&version='+Basiclytics.version+'&event='+event_type+'&data='+encodeURIComponent(JSON.stringify(data));
  img.onerror = function(err) {
    Basiclytics.debug("failed to sendData");
    Basiclytics.debug(err);
  };
};
